...
x_train = x_train.reshape(-1, 28, 28, 1).astype(np.float32) / 255.0  # ndarray로 변환, 차원 변경
...
# 신경망 모델 설계
cnn = Sequential()
cnn.add(Conv2D(32, (3,3), activation='relu', input_shape=(28,28,1)))  # 입력 형태 수정
...
for i in range(n_kernel):                       # i번째 커널
    f = kernel[:,:,0,i]  # 첫 번째 채널에 대해서만 시각화
